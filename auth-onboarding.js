(() => {
  ...
  function removeFirstAccessControls() {
    document.getElementById('xbFirstAccessBtn')?.remove();
    document.querySelectorAll('.xb-first-access-note').forEach(element => element.remove());
  }
  ...
  async function syncCredentialFields(client) {
    ...
    if (card && !card.querySelector('.xb-cloud-access-note')) {
      ...
      note.innerHTML = `${typeof icon === 'function' ? icon('shield-check') : ''}<span>O cadastro inicial já foi concluído. Use esta área somente para alterar o e-mail ou a senha da conta existente. As alterações valem em todos os aparelhos.</span>`;
  ...
  function installCredentialsBridge(client) {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        notify(changes.email
          ? 'Acesso atualizado. Se o Supabase solicitar, confirme o novo e-mail.'
  ...
  start().catch(error => console.error('[X-Burguer] Gestão de acesso Supabase:', error));
})();
